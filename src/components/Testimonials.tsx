...
const Testimonials = () => {
  ...
  return <section className="section-padding bg-white">
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-3xl mx-auto relative" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
          <div className="relative overflow-hidden min-h-[220px]">
            {testimonials.map((testimonial, index) => <div key={testimonial.id} className={`
          ...
          <div className="flex justify-center mt-6 gap-2">
            {testimonials.map((_, index) => <button key={index} onClick={() => handleDotClick(index)} className={`w-3 h-3 rounded-full transition-all ${index === activeIndex ? 'bg-gmarup-blue' : 'bg-gray-300'}`} aria-label={`עבור לביקורת ${index + 1}`} />)}
          </div>

          <div className="flex justify-between absolute top-1/2 right-0 left-0 -translate-y-1/2 px-2">
            <button onClick={handlePrev} className="w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gmarup-blue hover:bg-gray-50" aria-label="ביקורת קודמת">
              &rarr;
            </button>
            <button onClick={handleNext} className="w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-gmarup-blue hover:bg-gray-50" aria-label="ביקורת הבאה">
              &larr;
            </button>
          </div>
        </div>
